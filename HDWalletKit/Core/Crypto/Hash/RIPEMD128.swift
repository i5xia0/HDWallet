...
struct RIPEMD128 {
    ...
    private mutating func compress(_ X: UnsafePointer<UInt32>) {
        ...
        III(&ddd, aaa, &bbb, ccc, X[11], 13)
        III(&ccc, ddd, &aaa, bbb, X[ 3], 15)
        III(&bbb, ccc, &ddd, aaa, X[ 7],  7)
        III(&aaa, bbb, &ccc, ddd, X[ 0], 12)
        III(&ddd, aaa, &bbb, ccc, X[13],  8)
        III(&ccc, ddd, &aaa, bbb, X[ 5],  9)
        III(&bbb, ccc, &ddd, aaa, X[10], 11)
        III(&aaa, bbb, &ccc, ddd, X[14],  7)
        III(&ddd, aaa, &bbb, ccc, X[15],  7)
        III(&ccc, ddd, &aaa, bbb, X[ 8], 12)
        III(&bbb, ccc, &ddd, aaa, X[12],  7)
        III(&aaa, bbb, &ccc, ddd, X[ 4],  6)
        III(&ddd, aaa, &bbb, ccc, X[ 9], 15)
        III(&ccc, ddd, &aaa, bbb, X[ 1], 13)
        III(&bbb, ccc, &ddd, aaa, X[ 2], 11)
        
        /* parallel round 3 */
        HHH(&aaa, bbb, &ccc, ddd, X[15],  9)
        HHH(&ddd, aaa, &bbb, ccc, X[ 5],  7)
        HHH(&ccc, ddd, &aaa, bbb, X[ 1], 15)
        HHH(&bbb, ccc, &ddd, aaa, X[ 3], 11)
        HHH(&aaa, bbb, &ccc, ddd, X[ 7],  8)
        HHH(&ddd, aaa, &bbb, ccc, X[14],  6)
        HHH(&ccc, ddd, &aaa, bbb, X[ 6],  6)
        HHH(&bbb, ccc, &ddd, aaa, X[ 9], 14)
        HHH(&aaa, bbb, &ccc, ddd, X[11], 12)
        HHH(&ddd, aaa, &bbb, ccc, X[ 8], 13)
        HHH(&ccc, ddd, &aaa, bbb, X[12],  5)
        HHH(&bbb, ccc, &ddd, aaa, X[ 2], 14)
        HHH(&aaa, bbb, &ccc, ddd, X[10], 13)
        HHH(&ddd, aaa, &bbb, ccc, X[ 0], 13)
        HHH(&ccc, ddd, &aaa, bbb, X[ 4],  7)
        HHH(&bbb, ccc, &ddd, aaa, X[13],  5)
        
        /* parallel round 4 */
        GGG(&aaa, bbb, &ccc, ddd, X[ 8], 15)
        GGG(&ddd, aaa, &bbb, ccc, X[ 6],  5)
        GGG(&ccc, ddd, &aaa, bbb, X[ 4],  8)
        GGG(&bbb, ccc, &ddd, aaa, X[ 1], 11)
        GGG(&aaa, bbb, &ccc, ddd, X[ 3], 14)
        GGG(&ddd, aaa, &bbb, ccc, X[11], 14)
        GGG(&ccc, ddd, &aaa, bbb, X[15],  6)
        GGG(&bbb, ccc, &ddd, aaa, X[ 0], 14)
        GGG(&aaa, bbb, &ccc, ddd, X[ 5],  6)
        GGG(&ddd, aaa, &bbb, ccc, X[12],  9)
        GGG(&ccc, ddd, &aaa, bbb, X[ 2], 12)
        GGG(&bbb, ccc, &ddd, aaa, X[13],  9)
        GGG(&aaa, bbb, &ccc, ddd, X[ 9], 12)
        GGG(&ddd, aaa, &bbb, ccc, X[ 7],  5)
        GGG(&ccc, ddd, &aaa, bbb, X[10], 15)
        GGG(&bbb, ccc, &ddd, aaa, X[14],  8)
